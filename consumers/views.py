...
def backup_database(request):
    ...
    tables = [
        ...
        ('bills.json', Bill.objects.select_related('consumer').all()),
        ('payments.json', Payment.objects.select_related('consumer', 'bill').all()),
        ('meter_readings.json', MeterReading.objects.select_related('consumer').all()),
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        for filename, queryset in tables:
            try:
                data = dj_serializers.serialize('json', queryset, indent=2)
                zf.writestr(f'data/{filename}', data)
                total_records += queryset.count()
            except Exception as e:
                # Write an error note for this table instead of failing the entire backup
                zf.writestr(f'data/{filename}.ERROR.txt', f"Error exporting table: {str(e)}")
...
data/bills.json                — All billing records
data/payments.json             — All payment transactions
data/meter_readings.json       — All meter reading records
...
A Django management command can restore from these JSON fixtures.
...
"""
    ...
    zip_buffer.seek(0)
    filename = f'balilihan_waterworks_backup_{timestamp}.zip'
    ...
    try:
        ...
        UserActivity.objects.create(
            ...
            description=f"[BACKUP] Database backup downloaded: {filename} ({total_records} total records)",
            login_event=current_session,
