...
urlpatterns = [
    ...
    # Redirect root (/) to staff login (returns 200 for Render health check)
    path('', lambda request: HttpResponse("OK", status=200) if request.method == 'HEAD' or 'Go-http-client' in request.META.get('HTTP_USER_AGENT', '') else redirect('consumers:staff_login'), name='root_redirect'),
